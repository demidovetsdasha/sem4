...
def print_dnf(dnf):
    dnf_output = ""
    for i in dnf:
        dnf_output += f"({replece_params('*'.join(i))})+"
    print(f"DNF: {dereplece_params(dnf_output[:-1])}")


def print_knf(knf):
    knf_output = ""
    for i in knf:
        knf_output += f"({replece_params('+'.join(i))})*"
    print(f"KNF: {dereplece_params(knf_output[:-1])}")


def print_mdnf(mdnf):
    mdnf_output = ""
    for i in mdnf:
        mdnf_output += f"({replece_params('*'.join(i))})+"
    print(f"MDNF: {dereplece_params(mdnf_output[:-1])}")


def print_mknf(mknf):
    mknf_output = ""
    for i in mknf:
        mknf_output += f"({replece_params('+'.join(i))})*"
    print(f"MKNF: {dereplece_params(mknf_output[:-1])}")
